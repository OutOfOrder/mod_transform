...
#define GNOME_XSLT_FILTER "modxml-gnome-xslt"
...
static const char* gnome_cache_xslt(cmd_parms* cmd, void* cfg,
  ...
  svr_cfg* conf = ap_get_module_config(
	cmd->server->module_config, &xml_gnome_xslt_module ) ;
...
static apr_status_t gnome_xslt_run(ap_filter_t* f, xmlDocPtr doc) {
  ...
  modxml_notes* notes = ap_get_module_config(f->r->request_config, &xml_gnome_xslt_module) ;
...
static apr_status_t gnome_xslt_filter(ap_filter_t* f, apr_bucket_brigade* bb) {
  apr_bucket* b ;
  const char* buf = 0 ;
  apr_size_t bytes = 0 ;
...
static void gnome_xslt_hooks(apr_pool_t* p) {
  ...
  ap_register_output_filter(GNOME_XSLT_FILTER, gnome_xslt_filter, NULL, AP_FTYPE_RESOURCE) ;
